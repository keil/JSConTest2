...
(function () {
		...
		function buildPermissions (result, prefix, esOfPrefix, config) {
				var rev_suffixesOfES = getSuffixes (esOfPrefix);
				var rev_es = createESfromPaths (rev_suffixesOfES);
				var rev_suffixes = computePrefixes2 (rev_es, config.suffix_length, config.suffix_degree, config.suffix_maxlength);
		...
		// computes relevant prefixes as follows
		// if there is a step that does *not* decrease the number of property names,
		// then don't add it to the prefix!
		// TESTED
		function computePrefixes2 (es0, prefix_length) {
				var result = [];

				function comp (properties, path, es) {
						var np = properties.length;
						var ht = es.succ;
						result.push (cloneArray (path));

						var lp = {};
						var exists_p = false, all_p = true;
						for (var p in ht)
								if (ht.hasOwnProperty(p)) {
										var subproperties = propertyNamesOfES (ht[p]);
										lp[p] = subproperties;
										var llp = subproperties.length;
										if (np > llp) {
												exists_p = true;
										} else {
												all_p = false;
						...
						// exists_p does not make sense...
						if (path.length < prefix_length || all_p)
								for (var p in ht) 
										if (ht.hasOwnProperty (p)) {
												path.push(p);
												comp (lp[p], path, ht[p]);
				...
				comp (propertyNamesOfES(es0), [], es0);
				return result;
		}
		...
		function computePermissions (es, reduce, config) {
				var prefixes2 = computePrefixes2 (es, config.prefix_length);
				var reducedPrefixes2 = reduce (prefixes2, es);
				// console.log ("computePermissions", es, reducedPrefixes2);
				return computePermissionsFromPathSet (reducedPrefixes2, es, config);
		}
		...
		// helper for registering records from file
		...
		// when called, returns a record of hook methods
